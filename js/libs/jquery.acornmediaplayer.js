	...
	$.fn.acornMediaPlayer = function(options) {
		...
		var acornPlayer = function() {
			...
			*/
			...
			var $wrapper = $('<div class="acorn-player image-style" role="application"></div>').addClass(options.theme);
			 ...
			 * Complete markup
			 */
			var template = '<div class="acorn-controls" aria-live="off">' +
								'<input type="range" class="acorn-seek-slider" title="' + text.seekTitle + '" value="0" min="0" max="150" step="0.1" aria-controls="' + acorn.id + '"/>' +
								'<div class="acorn-play"><button class="acorn-play-button text-hide" title="' + text.playTitle + '" aria-controls="' + acorn.id + '">' + text.play + '</button></div>' +
								'<div class="timer-container small-5 columns">'+
								'<span class="acorn-timer" aria-hidden="true" >00:00</span>' +
								'<div class="video-length acorn-player.access"></div>'+
								'</div>'+
								'<div class="acorn-volume-box small-6 columns">' +
									'<button class="acorn-volume-button text-hide" title="' + text.mute + '" aria-controls="' + acorn.id + '">' + text.mute + '</button>' +
									'<input type="range" class="acorn-volume-slider" title="' + text.volumeTitle + '" value="1" min="0" max="1" step="0.05" aria-controls="' + acorn.id + '"/>' +
								'</div>' +
								fullscreenBtnMarkup +
								
								'<button class="acorn-caption-button hide" title="' + text.captionsTitle + '"  aria-controls="' + acorn.id + '">' + text.captions + '</button>' +
								'<div class="acorn-caption-selector"></div>' +
								'<button class="acorn-transcript-button btn-sprite btn-small" title="' + text.transcriptTitle + '">' + text.transcript + '</button>' +
							'</div>' +
							tooltipMarkup;


			
			var captionMarkup = '<div class="acorn-caption hide" aria-hidden="true"></div>';
			...
			acorn.$videoLength= $('.video-length', acorn.$container);
			...
			//acorn.$controls.after(videoLengthMarkup);
			/*
			...
			var updateSeek = function() {
				...
				if(options.nativeSliders) {
					...
					try{
						...
						//console.log('acorn.seekUpdate() slider Error');
					};
				...
				$.ajax({
					...
					success: function(data) {
						...
						$(captions).each(function() {
							//transcriptText += '<p data-begin="' + parseInt(this.start, 10) + '" data-end=' + parseInt(this.end, 10) + '>' + this.content.replace("'","") + '</p>';
							transcriptText += '<p>' + this.content.replace("'","") + '</p>';
			...
			var showTranscript = function() {
				...
				*/
				...
				acorn.$transcriptBtn.removeClass(transcriptBtnActiveClass);
			};
				...
				acorn.$self.bind('loadedmetadata', function() {
					...
					var t = window.setInterval(function() {
								if (acorn.$self.prop('readyState') > 0) {
									...
									updateSeek();
									 //Video length
									//acorn.$videoLength.html("&nbsp;/&nbsp;Duration: "+ariaTimeFormat(acorn.$self.prop('duration')));
									acorn.$videoLength.html("&nbsp;/&nbsp;"+timeFormat(acorn.$self.prop('duration')));
									clearInterval(t);
								}
							}, 500);
